...
export const clamp = (n: number) => Math.max(0.2, Math.min(0.8, n));
export function ids(n: LayoutNode | null): string[] {
  return !n ? [] : n.type === "leaf" ? [n.id] : [...ids(n.a), ...ids(n.b)];
}
...
export function grow(n: LayoutNode, id: string, delta: number): LayoutNode {
  if (n.type === "leaf") return n;
  if (n.a.type === "leaf" && n.a.id === id)
    return { ...n, ratio: clamp(n.ratio + delta) };
  if (n.b.type === "leaf" && n.b.id === id)
    return { ...n, ratio: clamp(n.ratio - delta) };
...
export function geometry(
  ...
  return n.dir === "row"
    ? [
        ...geometry(n.a, x, y, w * n.ratio, h),
        ...geometry(n.b, x + w * n.ratio, y, w * (1 - n.ratio), h),
      ]
    : [
        ...geometry(n.a, x, y, w, h * n.ratio),
        ...geometry(n.b, x, y + h * n.ratio, w, h * (1 - n.ratio)),
      ];
}
export function neighbor(
  ...
  const boxes = geometry(n),
    from = boxes.find((r) => r.id === id);
  if (!from) return null;
  ...
  return (
    boxes
      .filter((r) => r.id !== id)
      .map((r) => {
        const dx = r.x + r.w / 2 - (from.x + from.w / 2),
          dy = r.y + r.h / 2 - (from.y + from.h / 2);
        ...
        return {
          id: r.id,
          valid: main * sign > 0.001,
          score: Math.abs(main) + Math.abs(cross) * 2 + (overlap <= 0 ? 10 : 0),
        };
      })
      .filter((r) => r.valid)
