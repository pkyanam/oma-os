...
Workers AI integration adds `AI` and `AI_QUOTA`: no ChatGPT login is required, and inference is billed to the Cloudflare deployment. The quota Durable Object stores counters rather than prompts and reserves at most 16 calls/network/day and 64/deployment/day, with input/output and concurrency limits. Model selection is explicit; code integration is not evidence of a successful authenticated inference request. See [exact limits](DEPLOYMENT.md#workers-ai-mode) and [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/).
